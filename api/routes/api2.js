...
(function () {
        ...
        if(req.body.token){
            pool.connect(function (err, client, done) {
                if (err) {
                    ...
                    var query_string =  "select t.id, t.source, t.slot, t.price, t.sender, t.fee ,(select display_name from spezispezl.products where name= t.product), t.time, t.balance_new, t.transaction_id, t.sender, u.is_host from spezispezl.transactions t, spezispezl.user u where u.user_id = (select user_id from spezispezl.token where token = $1) and t.user_id = u.user_id and t.committed order by t.time desc";
                    var query = {
        ...
        if(true){
            pool.connect(function (err, client, done) {
                if (err) {
                    ...
                    var query_string =  "select name from spezispezl.products where visible = true order by name asc;";
                    var query = {
                    ...
                    client.query(query, async function (err, result) {
                        if (err || result.rows.length != 0) {
                            ...
                            var query = {
                                name: "create_user",
                                text: query_string,
                                values: [req.body.name.trim(), req.body.surname.trim(),  req.body.mail.toLowerCase().trim(), await hash_pw(req.body.passwd)],}
                            client.query(query, function (err, result) {
                                            ...
                                            client.query(query, function (err, result) {
                                                ...
                                                if (err) {
                                                    ...
                                                    let info = transporter.sendMail({
                                                        ...
                                                        to: req.body.mail.toLowerCase().trim(), // list of receivers
            ...
            pool.connect(function (err, client, done) {
                if (err) {
                    ...
                    var query = {
                        name: "find_user",
                        text: query_string,
                        values: [req.body.mail.toLowerCase().trim()],}
                    client.query(query, function (err, result) {
                        if(result && result.rows.length == 1){
            ...
            pool.connect(function (err, client, done) {
                if (err) {
                    ...
                    var query = {
                        ...
                        text: query_string,
                        values: [req.body.mail.trim().toLowerCase()],}
                    client.query(query, function (err, result) {
                        if (err) {
                            done();
                            return next(err);
                                ...
                                client.query(query, function (err, result) {
                                    ...
                                    if (err) {
                                        ...
                                        if(result && result.rows.length > 0){
                                            let info = transporter.sendMail({
                                            from: 'Spezispezl <spezispezl@mail.de>', // sender address
                                            to: req.body.mail.trim().toLowerCase(), // list of receivers
    ...
    router.post('/catering', function (req, res, next) { //Gästebewirtung
        res.setHeader('Access-Control-Allow-Origin', '*');
        res.setHeader('Access-Control-Allow-Methods', 'GET, POST, OPTIONS, PUT, PATCH, DELETE'); // If needed
        res.setHeader('Access-Control-Allow-Headers', 'X-Requested-With,contenttype'); // If needed
        res.setHeader('Access-Control-Allow-Credentials', true); // If needed
        
        if(req.body.token && req.body.id && req.body.msg){
            pool.connect(function (err, client, done) {
                if (err) {
                    done();
                    console.log(err);
                    ...
                    // user vom anfragetoken is_filler ("admin" kann für andere bewirtung freischalten)
                    // oder 
                    // user der zu stornierenden transaktion ist bewirtungsberechtigt (is_host) und sellt selbst die anfrage (token check)
                    var query_string = 'update spezispezl.transactions set committed = false where committed = true and id = $2 and \
                    ( ( (select user_id in (select u.user_id from spezispezl.user u where u.is_host)) and (user_id = (select user_id from spezispezl.token where token = $1)) ) \
                    or (select is_filler from spezispezl.user u where u.user_id = (select user_id from spezispezl.token where token = $1)) ) returning price, user_id, product, slot, parameter;';
                    var query = {
                        name: "do_transaction_storno",
                        text: query_string,
                        values: [req.body.token, req.body.id],}
                    client.query(query, function (err, result) {
                        if (err) {
                            ...
                            if(result && result.rows.length == 1){
                                var price = result.rows[0].price;
                                var user_id = result.rows[0].user_id;
                                var slot = result.rows[0].slot;
                                var product = result.rows[0].product;
                                var parameter = result.rows[0].parameter;
                                var query_string = "insert into spezispezl.transactions (user_id, source, product, transaction_id, price, balance_new, verified, committed) values ($1,'chargeback','chargeback_catering', $3, $2, (select balance from spezispezl.user where user_id= $1) +$2, true, true)";
                                var query = {
                                    name: "insert_transaction_storno",
                                    text: query_string,
                                    values: [user_id, -price, `${req.body.id} Gästebewirtung: ${req.body.msg}`],}
                                client.query(query, function (err, result) {
                                    if (err) {
                                        return next(err);
                                        res.json({ state: "fail"});
                                    } else {
                                        var query_string =  'update spezispezl.user set balance = balance-$1 where user_id = $2 returning balance;';
                                        var query = {
                                            name: "do_balance_storno",
                                            text: query_string,
                                        ...
                                        client.query(query, function (err, result) {
                                            if (err) {
                                                ...
                                                if(result && result.rows.length == 1){
                                                    var guest_user_id = 3;
                                                    var query_string = "insert into spezispezl.transactions (user_id, source, sender, slot, product, transaction_id, price, balance_new, verified, committed, parameter) values ($1,'catering',$5 ,$7, $4, $3, $2, (select balance from spezispezl.user where user_id= $1) +$2, true, true, $6)";
                                                    var query = {
                                                        name: "insert_transaction_catering",
                                                        text: query_string,
                                                        values: [guest_user_id, price, `${req.body.id}`, product, req.body.msg, parameter, slot],}
                                                    client.query(query, function (err, result) {
                                                        done();
                                                        if (err) {
                                                            return next(err);
                                ...
                                res.json({ state: "Transaktion nicht gefunden oder keine Berechtigung zur Gästebewirtung"});
                            }
                        }
                    });
                }
