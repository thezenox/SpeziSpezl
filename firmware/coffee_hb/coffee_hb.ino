...
#define PIN_C1 27 // coffee 1
#define PIN_C2 13 // coffee 2
#define PIN_C3 14 // manual
...
int8_t buttonpins[NUM_BUTTONS] ={PIN_C3, PIN_C1, PIN_C2};
...
// find free cart space and add product slot
  for(int i = 0; i< CARTSIZE; i++){
    if(!cart[i].valid){
      ...
      mill_time = products[slot].property;
  ...
  for(int i = 0; i< CARTSIZE; i++){
    if(cart[i].valid){
      if(!transaction_active){
        ...
        send_transaction(card_id, cart[i].p_id, (float)mill_time/1000);
        cart[i].valid = false;
      } else {
...
void send_transaction(String card_id, int slot, float time) { //, float time){
...
bool send_temp(float temp){
  ...
  String data = String("id=9&value=") +String(temp);
  return sendRequest(url, data);
}

bool send_restart(){
  String url = apiurl+ String("log_sensor");
  String data = String("id=10&value=") +String(get_reset_reason());
  return sendRequest(url, data);
}
