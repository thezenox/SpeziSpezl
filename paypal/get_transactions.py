...
def send_mail(mode, amount, mail, loaded, fee):
    from_email = config.from_email  # or simply the email address
    to_emails = [mail]
    if mode == 'insert':
        if fee > 0:
            body = 'Die Aufladung über {loaded:.2f}€ war erfolgreich.\nDein neues Guthaben beträgt {amount:.2f}€\n\nDir wurde die PayPal Gebühr von {fee:.2f}€ vom Aufladungsbetrag abgezogen. Um dies in Zukunft zu verhindern, wähle bitte bei PayPal "Familie & Freunde" aus.'.format(loaded = loaded, amount=amount, fee=fee)
        else:
...
def insert_transaction(comment, amount, fee, tid, name, surname):
    ...
    if len(user) > 0:
        ...
        if len(user) > 0:
            ...
            if len(user) == 1:
                ...
                qstr = '''select user_id from spezispezl."user" where lower(name)='{surname}' and lower(surname)='{name}'; '''.format(name = name.lower(), surname = surname.lower())
                print(qstr)
                user = cnx.execute(qstr).fetchall()
                #print(user)
                if len(user) == 1:
    ...
    if user_id:
        ...
        try: 
            ...
            if len(balance) > 0:
                send_mail('insert', balance[0][0], balance[0][1], amount, fee)
        ...
        send_mail('error', 0, config.supportmail, ["Can not find user ID for name: {name} surname: {surname} comment: {comment}".format(name=name, surname=surname, comment=comment)], 0)
    return False
...
def paypal_parse(html):
    ...
    if name and tid: #amount - fee > 0 and
        ...
        if name !='' or surname != '' or amount !=0: # found something but can not insert
            send_mail('error', 0, config.supportmail, ["PayPal parse error for name: {name} surname: {surname} comment: {comment}".format(name=name, surname=surname, comment=comment)],0)
...
def handle_mails():
    ...
    for i in range(messages, 0, -1):
        try:
            ...
            print(e)
            send_mail('error', 0, config.supportmail, e,0)
    imap.close()
    imap.logout()
